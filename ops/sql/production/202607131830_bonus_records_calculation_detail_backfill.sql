...
-- ENV: production
-- PURPOSE: Backfill historical bonus_records.calculation_detail with derived display snapshots.
--          This does not recalculate bonuses and does not change status, wallet, reward_wallet_logs,
--          point_transactions, release records, or settlement batches.
...
-- BACKUP_TABLES: ops_backup.bonus_records_calculation_detail_backfill_20260713
-- ROLLBACK: See SECTION: rollback. Restores backed-up calculation_detail values by bonus_records.id.
...
SELECT
  'backup_summary' AS check_name,
  COUNT(*) AS backed_up_rows,
  COUNT(*) FILTER (WHERE old_status = 'released') AS released_rows,
  COUNT(*) FILTER (WHERE old_status = 'waiting_release') AS waiting_release_rows,
  COUNT(*) FILTER (WHERE old_status = 'cancelled') AS cancelled_rows,
  COUNT(*) FILTER (WHERE old_status = 'failed') AS failed_rows,
  COUNT(*) FILTER (WHERE old_status NOT IN ('released','waiting_release','cancelled','failed')) AS other_status_rows
FROM ops_backup.bonus_records_calculation_detail_backfill_20260713;
...
WITH targets AS (
  SELECT
    ...
    br.bonus_rate,
    br.bonus_points,
    br.required_points_checked,
    br.required_points_passed,
    br.fail_reason,
    ...
    recipient.member_no AS recipient_member_no,
    recipient.is_vip AS recipient_is_vip,
    recipient.vip_expires_at AS recipient_vip_expires_at,
    recipient.is_dealer AS recipient_is_dealer,
    source_profile.name AS source_member_name,
    source_profile.member_no AS source_member_no,
    ...
    mrp.ym AS responsibility_ym,
    mrp.points AS responsibility_points,
  ...
  SELECT
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'bonus_record_snapshot', jsonb_strip_nulls(jsonb_build_object(
        ...
        'status', t.status,
        'generation_level', t.generation_level,
        ...
        'release_source', t.release_source,
        'release_attempts', t.release_attempts,
      ...
      'recipient_snapshot', jsonb_strip_nulls(jsonb_build_object(
        ...
        'member_name', t.recipient_name,
        'member_no', t.recipient_member_no,
        'is_vip', t.recipient_is_vip,
        'vip_expires_at', t.recipient_vip_expires_at,
        'is_dealer', t.recipient_is_dealer,
      ...
      'source_member_snapshot', jsonb_strip_nulls(jsonb_build_object(
        'member_id', t.source_member_id,
        'member_name', t.source_member_name,
        'member_no', t.source_member_no
      )),
      'source_order_snapshot', jsonb_strip_nulls(jsonb_build_object(
        'order_id', t.source_order_id,
        'order_no', t.source_order_no,
        'order_type', t.source_order_type,
        'total_amount', t.source_order_total_amount,
        'payment_status', t.source_order_payment_status,
        'created_at', t.source_order_created_at
      )),
      'settlement_batch_snapshot', jsonb_strip_nulls(jsonb_build_object(
        'batch_id', t.settlement_batch_id,
        'settlement_type', t.batch_settlement_type,
        'period_start', t.batch_period_start,
        'period_end', t.batch_period_end,
        'status', t.batch_status
      )),
      'responsibility_snapshot', jsonb_strip_nulls(jsonb_build_object(
        'ym', t.responsibility_ym,
        'points', t.responsibility_points,
...
-- SECTION: verify
SELECT
  'verify_missing_after_backfill' AS check_name,
  status,
  bonus_type,
  COUNT(*) AS rows_without_calculation_detail
FROM public.bonus_records
WHERE calculation_detail IS NULL
GROUP BY status, bonus_type
ORDER BY status, bonus_type;

SELECT
  'verify_backfilled_by_status' AS check_name,
  calculation_detail->>'status_classification' AS status_classification,
  status,
  bonus_type,
  COUNT(*) AS rows_with_backfill_snapshot,
  SUM(bonus_points) AS total_bonus_points
FROM public.bonus_records
WHERE calculation_detail->>'backfill_mode' = 'derived_from_existing_bonus_records'
GROUP BY calculation_detail->>'status_classification', status, bonus_type
ORDER BY status_classification, status, bonus_type;

SELECT
  'verify_affected_status_summary' AS check_name,
  old_status AS status,
  old_bonus_type AS bonus_type,
  COUNT(*) AS backed_up_rows
FROM ops_backup.bonus_records_calculation_detail_backfill_20260713
GROUP BY old_status, old_bonus_type
ORDER BY old_status, old_bonus_type;

SELECT
  'verify_sample' AS check_name,
  br.id,
  br.status,
  br.bonus_type,
  br.bonus_points,
  br.calculation_detail->>'calculation_scope' AS calculation_scope,
  br.calculation_detail->>'status_classification' AS status_classification,
  br.calculation_detail->>'exact_original_rule_snapshot' AS exact_original_rule_snapshot
FROM public.bonus_records br
WHERE br.calculation_detail->>'backfill_mode' = 'derived_from_existing_bonus_records'
ORDER BY br.created_at DESC
LIMIT 20;

-- SECTION: rollback
-- Manual rollback: restore calculation_detail values for rows captured by the backup step.
-- This intentionally does not change status, wallet, reward_wallet_logs, point_transactions, or settlement batches.
-- Uncomment the block below only if the verify step fails and the owner approves rollback.
--
-- BEGIN;
--
-- UPDATE public.bonus_records br
-- SET calculation_detail = b.old_calculation_detail
-- FROM ops_backup.bonus_records_calculation_detail_backfill_20260713 b
-- WHERE br.id = b.id
--   AND br.calculation_detail->>'backfill_mode' = 'derived_from_existing_bonus_records';
--
-- COMMIT;
--
-- SELECT
--   'rollback_verify_remaining_backfill_snapshots' AS check_name,
--   COUNT(*) AS rows_still_with_backfill_snapshot
-- FROM public.bonus_records
-- WHERE calculation_detail->>'backfill_mode' = 'derived_from_existing_bonus_records';

SELECT
  'rollback_definition_only_no_action_taken' AS rollback_status,
  COUNT(*) AS rollback_candidate_rows
FROM ops_backup.bonus_records_calculation_detail_backfill_20260713;
