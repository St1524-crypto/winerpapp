...
DECLARE
  ...
  _release_date date;
  _batch_id uuid;
  _default_rank public.rank_rebate_settings;
  _required int;
  _granted int := 0;
  _total_pts int := 0;
  _members int := 0;
  ...
  _rebate int;
BEGIN
  -- Authorization (skip service_role / cron)
  IF _uid IS NOT NULL
     AND NOT (
  ...
  SELECT * INTO _default_rank FROM public.rank_rebate_settings
    WHERE enabled = true ORDER BY sort_order LIMIT 1;
  _required := COALESCE(_default_rank.required_points, _s.vip_required_points);
  ...
  LOOP
    ...
    -- 超額回饋（以個人責任額為基礎）
    IF _passed AND _default_rank.id IS NOT NULL
       AND _self_pts > _required
       AND COALESCE(_default_rank.exceeded_rebate_rate, 0) > 0 THEN
      _excess := _self_pts - _required;
      _rebate := floor(_excess * _default_rank.exceeded_rebate_rate / 100)::int;
      IF _rebate > 0 THEN
        INSERT INTO public.bonus_records(
          member_id, bonus_type, base_amount, bonus_rate, bonus_points,
          ...
          _vip.id, 'rank_rebate', _excess, _default_rank.exceeded_rebate_rate, _rebate,
