-- Atomic order creation with discount / shopping / reward point offsets.
-- This adds a new RPC and intentionally leaves create_sales_order_with_items unchanged.
...
BEGIN
  ...
  IF _point_payments IS NULL THEN
    _point_payments := '[]'::jsonb;
  END IF;
  ...
  IF jsonb_array_length(_point_payments) > 0 THEN
    ...
    LOOP
      ...
      IF _point_type = 'discount' THEN
        _balance_after := _discount_balance;
      ELSIF _point_type = 'shopping' THEN
        _balance_after := _shopping_balance;
      ELSE
        _balance_after := _reward_balance;
      END IF;
...
GRANT EXECUTE ON FUNCTION public.create_sales_order_with_point_payments(jsonb, jsonb, jsonb, jsonb) TO authenticated;
