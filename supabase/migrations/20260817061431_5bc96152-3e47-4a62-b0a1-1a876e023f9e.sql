ALTER TABLE public.sales_orders
  ADD COLUMN IF NOT EXISTS no_reward_points boolean NOT NULL DEFAULT false;

CREATE OR REPLACE FUNCTION public.create_sales_order_with_point_payments(_order jsonb, _items jsonb, _payments jsonb DEFAULT '[]'::jsonb, _point_payments jsonb DEFAULT '[]'::jsonb)
...
BEGIN
  ...
  _no_reward := _is_staff AND COALESCE((_order->>'no_reward_points')::boolean, false);
  ...
  INSERT INTO public.sales_orders (
    ...
    order_status, shipping_status, payment_status, company_id, no_reward_points
  )
  VALUES (
    _order->>'order_no',
    _member_id,
    ...
    COALESCE(NULLIF(_order->>'shipping_method',''), 'home_delivery'),
    GREATEST(COALESCE((_order->>'subtotal')::numeric, 0), 0),
    _shipping_fee,
    GREATEST(COALESCE((_order->>'discount_amount')::numeric, 0), 0),
    GREATEST(COALESCE((_order->>'total_amount')::numeric, 0), 0),
    NULLIF(_order->>'notes',''),
    COALESCE(NULLIF(_order->>'order_status',''), 'pending'),
    COALESCE(NULLIF(_order->>'shipping_status',''), 'pending'),
    COALESCE(NULLIF(_order->>'payment_status',''), 'pending'),
    _company_id,
    _no_reward
  )
  ...
  INSERT INTO public.sales_order_items (
    sales_order_id, product_id, product_name, sku, image, unit_price, quantity, subtotal, company_id, is_gift, tier_reward_points
  )
  SELECT
    new_order.id,
    x.product_id,
    ...
    CASE WHEN _no_reward THEN 0 ELSE NULL END
  FROM (
    SELECT
  ...
  IF jsonb_array_length(_point_payments) > 0 THEN
    ...
    LOOP
      ...
      _dedupe_key := 'order:' || new_order.id::text || ':' || _point_type;

      INSERT INTO public.point_transactions (
        user_id, point_type, amount, balance_after, reason, reference_type, reference_id, company_id
      )
      VALUES (
        _member_id,
        _point_type,
        -_points_used,
        _balance_after,
        COALESCE(NULLIF(_point_payment->>'note',''), '訂單折抵'),
        'sales_order',
        new_order.id,
        _company_id
      )
      ...
      INSERT INTO public.order_point_payments (
        sales_order_id, user_id, point_type, points_used, amount_offset,
        balance_after, point_transaction_id, company_id, dedupe_key, note
      )
      VALUES (
        new_order.id,
        _member_id,
  ...
  IF jsonb_array_length(_payments) > 0 THEN
    ...
    SELECT
      ...
      COALESCE(NULLIF(p->>'payment_status',''), 'paid'),
