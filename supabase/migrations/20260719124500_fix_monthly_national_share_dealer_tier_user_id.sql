-- Fix monthly national-share settlement tier lookup.
--
-- public.dealer_tier_status uses user_id, not member_id.  The existing
-- settle_monthly_national_share function still referenced s.member_id while
-- reading dealer_tier_status, causing monthly recalculation dry-runs to fail.
--
-- This migration only rewrites the function definition. It does not execute
-- monthly settlement, does not update bonus_records, and does not touch wallet
-- or point ledgers.
