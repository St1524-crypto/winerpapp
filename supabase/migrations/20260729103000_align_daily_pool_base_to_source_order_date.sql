...
-- This fixes the 03:00 TW cron date shift where an order from the previous
-- business day is settled on the next calendar day.
...
DECLARE
  ...
  _revenue text;
BEGIN
  SELECT pg_get_functiondef('public.calculate_daily_order_reward_points_by_source_date(date)'::regprocedure)
    INTO _helper;
  SELECT pg_get_functiondef('public.distribute_daily_revenue_bonus(date)'::regprocedure)
    INTO _revenue;

  IF _helper IS NULL THEN
    RAISE EXCEPTION 'Verification failed: helper function missing';
  END IF;

  IF position('GROUP BY br.source_order_id' in _helper) = 0 THEN
    RAISE EXCEPTION 'Verification failed: helper must dedupe by source_order_id';
  END IF;

  IF position('(so.created_at AT TIME ZONE ''Asia/Taipei'')::date = _source_date' in _helper) = 0 THEN
    RAISE EXCEPTION 'Verification failed: helper must align by source order Taiwan date';
  END IF;

  IF position('public.calculate_daily_order_reward_points_by_source_date(_date)' in _revenue) = 0 THEN
    RAISE EXCEPTION 'Verification failed: revenue bonus must use source-date helper';
  END IF;

  IF position('FROM public.point_transactions' in _revenue) > 0 THEN
    RAISE EXCEPTION 'Verification failed: revenue bonus still reads point_transactions directly';
  END IF;

  IF position('''E'',''A''' in _revenue) > 0 THEN
    RAISE EXCEPTION 'Verification failed: revenue bonus must not include V/S/T/E/A consumption tiers';
  END IF;

  IF position('s.user_id' in _revenue) = 0 THEN
    RAISE EXCEPTION 'Verification failed: revenue bonus must use dealer_tier_status.user_id';
  END IF;
END $$;
