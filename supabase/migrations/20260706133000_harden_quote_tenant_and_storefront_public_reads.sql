...
-- This migration is intentionally limited to RLS/GRANT/view boundaries.

-- 1) Quote company settings: staff can only access their current company.
...
-- 2) Quote bank accounts: staff can only access their current company.
...
-- 3) Quotes: remove broad token reads and scope every staff path by company.
...
-- 4) Quote items inherit quote tenant scope.
  ...
  EXISTS (
    ...
    WHERE q.id = quote_items.quote_id
      AND (
        private.has_role(auth.uid(), 'super_admin'::app_role)
        OR (
          q.company_id = private.current_company_id()
          AND (
            private.has_role(auth.uid(), 'admin'::app_role)
            OR private.has_role(auth.uid(), 'finance'::app_role)
          )
...
-- 5) Public member custom products: do not expose raw member_id to anonymous users.
...
-- dealer_program_settings is intentionally platform-wide at the current schema level.
-- It has no company_id column, and write access remains limited by existing staff policies.
