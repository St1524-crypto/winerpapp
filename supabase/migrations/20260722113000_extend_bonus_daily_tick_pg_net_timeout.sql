-- fix(cron): extend bonus daily tick pg_net timeout
--
-- Purpose:
--   pg_net currently records bonus-daily-tick as timed out after the default
--   5000 ms, even when the endpoint finishes and advances daily_next_settlement_at.
--   Increase only the HTTP wait timeout to 30000 ms so cron diagnostics can
--   capture the response body.
--
-- Safety:
--   - Does not call the endpoint.
--   - Does not run daily settlement.
--   - Does not modify bonus_records, wallets, ledgers, point_transactions, or settings.
--   - Does not expose the cron token; command keeps using the Vault helper.
...
-- SECTION: rollback
-- Manual rollback: restore jobid 137 command/schedule/active from backup.
-- DO $$
-- DECLARE
--   b record;
-- BEGIN
--   SELECT * INTO b
--   FROM ops_backup.bonus_daily_tick_cron_20260722_timeout_30s
--   WHERE jobid = 137;
--
--   IF b.jobid IS NULL THEN
