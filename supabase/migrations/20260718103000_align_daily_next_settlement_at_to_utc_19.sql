...
--
-- Purpose:
--   settle_daily_bonus previously advanced daily_next_settlement_at using now()
--   plus the cycle interval. That preserved execution seconds, so a cron at
--   exactly 19:00:00 UTC could arrive a few seconds before the next threshold
--   and skip daily settlement. This patch keeps the existing bonus logic and
--   changes only the next-run timestamp calculation.
--
-- Safety:
