-- Harden user-writable financial and operation task records.
-- Scope: RLS / trigger defenses only. No UI or payment flow changes.

-- 1) Cash transactions: members may only create pending top-up/withdraw requests.
-- Server/admin flows continue to use service_role or admin/finance policies.
...
-- 2) Operation tasks: assigned staff may update only their status.
-- Managers/admins keep existing management powers.
