...
function CategoryPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: p } = await supabase.from("products").select(PRODUCT_PUBLIC_COLUMNS).eq("category_id", slug).eq("status", "active").order("display_priority", { ascending: false }).order("created_at", { ascending: false });
