...
import {
  aggregateSplit, DAILY_COLUMN_MAP, DAILY_TEMPLATE_COLUMNS,
  exportSummaryXls, fmtN, type SummaryRow,
} from "@/lib/bonus-report-shared";

const ALLOWED: AppRole[] = ["super_admin", "admin", "finance"];
export const Route = createFileRoute("/_authenticated/admin/bonuses/daily-summary-split")({ component: Guard });

function Guard() {
  const { roles, loading } = useAuth();
  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  if (!roles.some((r) => ALLOWED.includes(r))) return <ForbiddenScreen requiredRoles={ALLOWED} pageName="日獎金總表（分開）" />;
  return <Page />;
}
...
function Page() {
  ...
  const rows: SummaryRow[] = useMemo(() => {
    if (!payload) return [];
    return aggregateSplit(payload.rows ?? [], payload.members ?? {}, DAILY_COLUMN_MAP, DAILY_TEMPLATE_COLUMNS);
  }, [payload]);
  ...
  function onExport() {
    ...
    exportSummaryXls({ periodLabel: period, templateCols: DAILY_TEMPLATE_COLUMNS, rows, filename: "日獎金總表-分開", scope: "daily" });
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 print:max-w-none">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">日獎金總表（分開）</h1>
          <p className="mt-1 text-sm text-muted-foreground">同會員每個獎金類型各佔一列，欄位對齊「日獎金總表分開.xls」。</p>
        </div>
      ...
      <Card className="border-amber-500/50 bg-amber-500/10 print:hidden">
        <CardContent className="py-3 flex items-start gap-2 text-xs">
          ...
          <div>身份証號 / 地址 / 証號別欄位目前 DB 尚未儲存；稅額（5% / 10% / 健保費）為報表估算，僅供對帳呈現，不影響實際發放。</div>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle className="text-base">獎金期間：{period}（共 {rows.length} 列）</CardTitle></CardHeader>
        <CardContent>
...
function ReportTable({ rows }: { rows: SummaryRow[] }) {
  return (
