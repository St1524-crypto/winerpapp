      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>{formIsBusinessDividendTier ? "營業分紅 %" : "營業分紅 %（V/S/T/E/A 不適用）"}</Label><Input type="number" step="0.01" value={form.revenue_share_rate} disabled={!formIsBusinessDividendTier} onChange={(e) => setForm({ ...form, revenue_share_rate: e.target.value })} /></div>
            <div><Label>{capLabel(form)}</Label><Input type="number" value={form.upgrade_bonus_cap} onChange={(e) => setForm({ ...form, upgrade_bonus_cap: e.target.value })} /></div>
