...
export const Route = createFileRoute("/_authenticated/admin/bonuses/monthly-summary")({ component: Guard });

function Guard() {
  const { roles, loading } = useAuth();
  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  if (!roles.some((r) => ALLOWED.includes(r))) return <ForbiddenScreen requiredRoles={ALLOWED} pageName="月獎金總表" />;
  return <Page />;
}
...
function Page() {
  ...
  function onExport() {
    ...
    exportSummaryXls({ periodLabel: period, templateCols: MONTHLY_TEMPLATE_COLUMNS, rows, filename: "月獎金總表", scope: "monthly" });
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 print:max-w-none">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">月獎金總表</h1>
          <p className="mt-1 text-sm text-muted-foreground">同會員同月份彙總為一列，欄位對齊「月獎金總表.xls」。</p>
        </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {MONTHLY_TEMPLATE_COLUMNS.map((c) => <TableHead key={c} className="text-right">{c}</TableHead>)}
                  ...
                  <TableBody>
                    {rows.map((r) => (
                      <TableRow key={r.memberId}>
                        ...
                        {MONTHLY_TEMPLATE_COLUMNS.map((c) => <TableCell key={c} className="text-right tabular-nums">{fmtN(r.columns[c])}</TableCell>)}
