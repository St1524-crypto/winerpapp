...
import {
  adminListMembersBusinessBonusSummary,
  adminListBusinessBonusLedger,
  previewBusinessBonusRelease,
} from "@/lib/vip-business-bonus-cap.functions";
...
function Page() {
  const listSummary = useServerFn(adminListMembersBusinessBonusSummary);
  const listLedger = useServerFn(adminListBusinessBonusLedger);
  const preview = useServerFn(previewBusinessBonusRelease);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>計算試算（不會寫入）</CardTitle></CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-4">
          ...
          {result && (
            <div className="md:col-span-4 grid grid-cols-2 md:grid-cols-7 gap-2 text-sm">
              <Field label="原應發" value={fmt(result.bonus_amount)} />
              <Field label="實發" value={fmt(result.payable_amount)} />
              <Field label="截斷" value={fmt(result.capped_amount)} />
              <Field label="發前累計" value={fmt(result.total_before)} />
              <Field label="發後累計" value={fmt(result.total_after)} />
              <Field label="上限" value={fmt(result.cap_amount)} />
      ...
      <Card>
        <CardHeader><CardTitle>會員累計 / 上限 / 剩餘可領</CardTitle></CardHeader>
        <CardContent>
          {loading ? <p className="text-muted-foreground">載入中…</p> :
            summary.length === 0 ? <p className="text-muted-foreground">尚無紀錄</p> : (
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>會員</TableHead>
                    <TableHead>階級</TableHead>
                    <TableHead>分紅比例</TableHead>
                ...
                <TableBody>
                  {summary.map((m) => (
                    <TableRow key={m.memberId}>
                      ...
                      <TableCell>{pct(m.rate)}</TableCell>
                ...
                <TableBody>
                  {ledger.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-xs">{fmt(r.total_before)} → {fmt(r.total_after)}</TableCell>
