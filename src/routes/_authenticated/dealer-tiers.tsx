          ...
          {editing && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label={isBusinessDividendTier(editing) ? "營業分紅率 %" : "營業分紅率 %（V/S/T/E/A 不適用）"}><Input type="number" step="0.01" value={editing.operating_bonus_rate} disabled={!isBusinessDividendTier(editing)} onChange={(e) => setEditing({ ...editing, operating_bonus_rate: +e.target.value })} /></Field>
                <Field label={isBusinessDividendTier(editing) ? "營業分紅上限" : "消費回饋上限"}><Input type="number" value={editing.upgrade_bonus_cap} onChange={(e) => setEditing({ ...editing, upgrade_bonus_cap: +e.target.value })} /></Field>
